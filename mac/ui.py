...
Spusti na Mac M4 ako: python mac/ui.py  (z korena repozitara)

Zavislosti:
  pip install customtkinter httpx obsws-python mido python-rtmidi zeroconf
  + ffmpeg v PATH (pre pripadne lokalne testy)
  + MediaMTX bezi lokalne: ./mediamtx mac/mediamtx.yml
"""

import sys
from pathlib import Path

_mac_dir = Path(__file__).resolve().parent
if str(_mac_dir) not in sys.path:
    sys.path.insert(0, str(_mac_dir))
